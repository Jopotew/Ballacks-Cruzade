...
func draw() -> void:
    ...
    if hand_size < MAX_HAND_SIZE:
        ...
        var card_node = cards.instantiate()  
        live_hand.append({
            "scene": cards,  
            "node": card_node 
        })
...
func add_card_to_hand(new_card_data) -> void:
    
    var card_action = new_card_data["action"]
    var card_text = new_card_data["text"]
    var card_name = new_card_data["card_name"]
